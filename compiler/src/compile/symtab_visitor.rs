...
pub struct StructLayout {
    ...
    pub fields: Vec<FieldAccessor>,
}

/// How a single field is accessed in storage.
#[derive(Clone, Debug)]
pub enum FieldAccessor {
    /// Local register storage - use Move instruction.
    Local(SymbolId),
...
impl StructLayout {
    /// Create a StructLayout for a local variable.
    /// Creates symbols for each field and builds nested layouts recursively.
    pub fn for_local(
        struct_id: StructId,
        base_name: &str,
        span: Span,
    ...
    /// Create a StructLayout for a property.
    /// Uses property indices for scalar fields and builds nested layouts recursively.
    pub fn for_property(
        struct_id: StructId,
        base_name: &str,
        ...
        for field in &def.fields {
            ...
            match field.ty {
                ...
                _ => {
                    ...
                    fields.push(FieldAccessor::Property(*base_index));
                    *base_index += 1;
                }
            }
        }
    ...
    /// Create a StructLayout for a global variable.
    /// Uses GlobalIds for scalar fields and builds nested layouts recursively.
    pub fn for_global(
        struct_id: StructId,
        base_name: &str,
        base_index: &mut usize,
        ...
        for field in &def.fields {
            ...
            match field.ty {
                ...
                _ => {
                    ...
                    fields.push(FieldAccessor::Global(GlobalId(*base_index)));
                    *base_index += 1;
                }
            }
        }
    ...
    /// Collect all leaf (scalar) accessors from this layout recursively.
    pub fn leaf_accessors(&self) -> Vec<&FieldAccessor> {
    ...
    /// Collect all leaf symbol IDs from Local accessors.
    /// Useful for collecting argument/return symbols.
    pub fn leaf_symbols(&self) -> Vec<SymbolId> {
        let mut symbols = Vec::new();
        self.collect_leaf_symbols(&mut symbols);
        symbols
    }

    fn collect_leaf_symbols(&self, out: &mut Vec<SymbolId>) {
        for field in &self.fields {
            match field {
    ...
    /// Navigate to a field by following a path of field indices.
    /// Returns None if the path is invalid or points to a scalar.
    pub fn navigate(&self, path: &[usize]) -> Option<&FieldAccessor> {
        if path.is_empty() {
            return None;
        }

        let field = self.fields.get(path[0])?;
...
impl<'input> SymTabVisitor<'input> {
    pub fn new(
        ...
        // Create StructLayouts for struct property bases
        // We iterate through declared properties to find struct-typed ones
        for decl in &script.property_declarations {
            let prop_ty = decl.name.ty_required().resolved();
            if let Type::Struct(struct_id) = prop_ty {
                let name = decl.name.name();
                // Find the starting property index for this struct
                if let Some(first_prop) = properties.iter().find(|p| {
                    p.struct_info
                        .as_ref()
...
impl<'input> NameTable<'input> {
    ...
    pub fn get(&self, name: &str, symtab: &SymTab) -> Option<SymbolId> {
        ...
        if let Some(symbol_id) = symtab.get_property_base_symbol(name) {
...
pub struct SymTab<'input> {
    ...
    /// Maps property base names to their symbol IDs.
    /// Used during variable resolution to return the property base symbol.
    property_base_symbols: HashMap<String, SymbolId>,
}
...
impl<'input> SymTab<'input> {
    ...
    /// Register a struct layout for a symbol.
    pub fn register_struct_layout(&mut self, symbol_id: SymbolId, layout: StructLayout) {
    ...
    /// Returns a clone of the layout (needed because creation mutates self).
    pub fn ensure_local_layout(
        &mut self,
        symbol_id: SymbolId,
        struct_id: StructId,
        span: Span,
        ...
        if let Some(layout) = self.struct_layouts.get(&symbol_id) {
            return layout.clone();
        }
        let name = self.name_for_symbol(symbol_id).to_string();
        let layout = StructLayout::for_local(struct_id, &name, span, self, registry);
        self.struct_layouts.insert(symbol_id, layout.clone());
        layout
    }
    ...
    pub fn get_property_base_symbol(&self, name: &str) -> Option<SymbolId> {
        self.property_base_symbols.get(name).copied()
    }

    /// Check if a symbol is a property base.
    pub fn is_property_base(&self, symbol_id: SymbolId) -> bool {
        self.property_base_struct_ids.contains_key(&symbol_id)
    }

    /// Get the struct ID for a property base symbol.
    pub fn get_property_base_struct_id(&self, symbol_id: SymbolId) -> Option<StructId> {
    ...
    pub fn get_storage(&self, symbol_id: SymbolId) -> SymbolStorage {
        ...
        // Check for struct property base
        if self.property_base_struct_ids.contains_key(&symbol_id) {
            // Struct property bases don't have direct storage - they're resolved to field paths
            // Treat as local (the caller should handle struct property bases specially)
    ...
    pub(crate) fn name_for_symbol(&self, symbol_id: SymbolId) -> Cow<'input, str> {
        ...
        // Regular symbol lookup (includes property bases which are now real symbols)
    ...
    pub(crate) fn span_for_symbol(&self, symbol_id: SymbolId) -> Span {
        // Property bases and regular symbols have spans in symbol_names
...
mod test {
    ...
    #[test]
    fn struct_layout_matches_global_struct_expansion() {
        let file_id = FileId::new(0);
        let input = r#"
            struct Point { x: int, y: int }
            global pos = Point(10, 20);
    ...
    #[test]
    fn struct_layout_matches_nested_global_struct() {
        let file_id = FileId::new(0);
        let input = r#"
            struct Point { x: int, y: int }
            struct Rect { origin: Point, size: Point }
            ...
            match field {
                FieldAccessor::Nested(_, nested) => {
                    ...
                    for f in &nested.fields {
                        match f {
                            FieldAccessor::Global(_) => {} // Expected
                            _ => panic!("Expected Global accessor in nested struct"),
                        }
                    }
                }
        ...
        // Leaf accessors should give 4 globals
    ...
    #[test]
    fn struct_layout_for_property_matches_struct_property_base() {
        let file_id = FileId::new(0);
        let input = r#"
            struct Point { x: int, y: int }
            property pos: Point;
